...
let SearchSelectJS = function( elem )
{
    ...
    let titre = '';
    if( elem.dataset.titre )
    {
        titre = elem.dataset.titre;
    }
    else
    {
...
SearchSelectJS.staticInit  = function(){
    ...
    //window.addEventListener('resize', SearchSelectJS.onResize);
    setInterval(SearchSelectJS.onResize,100);
    //window.addEventListener('scroll',SearchSelectJS.onResize);
}
